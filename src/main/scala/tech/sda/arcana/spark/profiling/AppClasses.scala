...
class Category (var Category: String,var uri:  List[RDFURI] ) {
  ...
  // Question processing
  case class QuestionObj(sentence:String,sentenceWoSW:String,SentimentExtraction:Int,tokens:List[Token],PosSentence:String,var phaseTwoScore:Int,var summary:String)
  case class Token(index:String,word:String,posTag:String,lemma:String,var relationID:Int,var uriTuple: List[(String,Double)],var tokenUrisSentiScore: Double)
